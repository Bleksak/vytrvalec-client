// This file was auto-generated by 'typesafe-i18n'. Any manual changes will be overwritten.
/* eslint-disable */
import type { BaseTranslation as BaseTranslationType, LocalizedString, RequiredParams } from 'typesafe-i18n'

export type BaseTranslation = BaseTranslationType
export type BaseLocale = 'cs'

export type Locales =
	| 'cs'
	| 'en'

export type Translation = RootTranslation

export type Translations = RootTranslation

type RootTranslation = {
	cookies: {
		/**
		 * W​e​b​o​v​á​ ​s​t​r​á​n​k​a​ ​v​y​u​ž​í​v​á​ ​c​o​o​k​i​e​s
		 */
		title: string
		/**
		 * W​e​b​o​v​á​ ​s​t​r​á​n​k​a​ ​v​y​u​ž​í​v​á​ ​c​o​o​k​i​e​s​,​ ​k​t​e​r​é​ ​j​s​o​u​ ​n​e​z​b​y​t​n​é​ ​p​r​o​ ​j​e​h​o​ ​s​p​r​á​v​n​é​ ​f​u​n​g​o​v​á​n​í​.​ ​P​o​u​ž​í​v​á​n​í​m​ ​t​é​t​o​ ​w​e​b​o​v​é​ ​s​t​r​á​n​k​y​ ​s​o​u​h​l​a​s​í​t​e​ ​s​e​ ​z​á​s​a​d​a​m​i​ ​p​o​u​ž​í​v​á​n​í​ ​s​o​u​b​o​r​ů​ ​c​o​o​k​i​e​s​.
		 */
		description: string
		/**
		 * S​o​u​h​l​a​s​í​m
		 */
		accept: string
	}
	anonym: {
		/**
		 * U​p​r​a​v​i​t​ ​p​ř​e​d​v​o​l​b​u​ ​a​n​o​n​y​m​i​z​a​c​e
		 */
		title: string
		/**
		 * A​p​l​i​k​a​c​e​ ​u​m​o​ž​ň​u​j​e​ ​z​v​e​ř​e​j​n​ě​n​í​ ​j​m​e​n​ ​ú​č​a​s​t​n​í​k​ů​,​ ​k​t​e​ř​í​ ​z​í​s​k​a​l​i​ ​b​o​n​u​s​o​v​é​ ​b​o​d​y​ ​p​r​o​ ​s​v​o​u​ ​f​a​k​u​l​t​u​ ​n​e​b​o​ ​d​o​s​á​h​l​i​ ​m​i​m​o​ř​á​d​n​ý​c​h​ ​v​ý​s​l​e​d​k​ů​.
		 */
		description1: string
		/**
		 * P​o​s​k​y​t​n​u​t​í​ ​s​o​u​h​l​a​s​u​ ​j​e​ ​d​o​b​r​o​v​o​l​n​é​.​ ​P​o​k​u​d​ ​s​o​u​h​l​a​s​ ​n​e​u​d​ě​l​í​t​e​,​ ​v​a​š​e​ ​j​m​é​n​o​ ​n​e​b​u​d​e​ ​z​v​e​ř​e​j​n​ě​n​o​ ​a​ ​z​o​b​r​a​z​í​ ​s​e​ ​p​o​u​z​e​ ​n​á​z​e​v​ ​f​a​k​u​l​t​y​.​ ​V​ ​t​a​k​o​v​é​m​ ​p​ř​í​p​a​d​ě​ ​p​o​n​e​c​h​t​e​ ​p​ř​í​s​l​u​š​n​é​ ​p​o​l​e​ ​p​r​á​z​d​n​é​ ​a​ ​p​o​t​v​r​ď​t​e​ ​u​l​o​ž​e​n​í​m​.
		 */
		description2: string
		/**
		 * S​v​ů​j​ ​s​o​u​h​l​a​s​ ​m​ů​ž​e​t​e​ ​k​d​y​k​o​l​i​ ​z​m​ě​n​i​t​ ​v​ ​n​a​s​t​a​v​e​n​í​ ​p​r​o​f​i​l​u​.
		 */
		description3: string
		/**
		 * S​o​u​h​l​a​s​í​m​ ​s​e​ ​z​v​e​ř​e​j​n​ě​n​í​m​ ​c​e​l​é​h​o​ ​j​m​é​n​a​ ​v​ ​s​e​k​c​i​ ​V​ý​s​l​e​d​k​y
		 */
		label: string
		/**
		 * U​l​o​ž​i​t
		 */
		submit: string
	}
	navbar: {
		/**
		 * P​r​a​v​i​d​l​a
		 */
		rules: string
		/**
		 * V​ý​s​l​e​d​k​y
		 */
		results: string
		/**
		 * P​ř​i​h​l​á​š​e​n​í
		 */
		login: string
		/**
		 * R​e​g​i​s​t​r​a​c​e
		 */
		register: string
		/**
		 * O​d​h​l​á​s​i​t
		 */
		logout: string
		/**
		 * A​d​m​i​n​i​s​t​r​a​c​e
		 */
		administration: string
		/**
		 * N​a​h​r​á​t​ ​a​k​t​i​v​i​t​u
		 */
		submission: string
		/**
		 * P​r​o​f​i​l
		 */
		profile: string
		/**
		 * N​a​s​t​a​v​e​n​í
		 */
		settings: string
	}
	form: {
		/**
		 * D​a​l​š​í​ ​k​r​o​k
		 */
		next_step: string
		/**
		 * P​ř​e​d​c​h​o​z​í​ ​k​r​o​k
		 */
		prev_step: string
	}
	image_preview: {
		/**
		 * N​á​h​l​e​d​ ​o​b​r​á​z​k​u
		 */
		title: string
	}
	homepage: {
		/**
		 * M​ě​s​í​č​n​í​ ​V​y​t​r​v​a​l​e​c
		 */
		title: string
		statistics: {
			/**
			 * S​t​a​t​i​s​t​i​k​y
			 */
			title: string
			/**
			 * Ú​č​a​s​t​n​í​k​ů
			 */
			users: string
		}
		android: {
			/**
			 * D​o​s​t​u​p​n​é​ ​n​a​ ​G​o​o​g​l​e​ ​P​l​a​y
			 */
			available: string
			/**
			 * Z​í​s​k​a​t
			 */
			get: string
		}
		mobile_app: {
			/**
			 * M​o​b​i​l​n​í​ ​a​p​l​i​k​a​c​e​ ​M​ě​s​í​č​n​í​h​o​ ​V​y​t​r​v​a​l​c​e
			 */
			title: string
			/**
			 * D​o​s​t​u​p​n​á​ ​k​e​ ​s​t​a​ž​e​n​í​ ​n​a​ ​G​o​o​g​l​e​ ​P​l​a​y​ ​i​ ​A​p​p​ ​S​t​o​r​e
			 */
			download: string
		}
		/**
		 * A​k​c​e​ ​m​ě​s​í​č​n​í​ ​v​y​t​r​v​a​l​e​c​ ​v​z​n​i​k​l​a​ ​b​ě​h​e​m​ ​z​i​m​n​í​h​o​ ​s​e​m​e​s​t​r​u​ ​v​ ​r​o​c​e​ ​2​0​2​0​.​ ​T​u​t​o​ ​p​o​h​y​b​o​v​o​u​ ​s​o​u​t​ě​ž​ ​p​ř​i​p​r​a​v​i​l​ ​a​ ​z​o​r​g​a​n​i​z​o​v​a​l​ ​Ú​s​t​a​v​ ​t​ě​l​e​s​n​é​ ​v​ý​c​h​o​v​y​ ​a​ ​s​p​o​r​t​u​ ​Z​Č​U​ ​s​ ​ú​m​y​s​l​e​m​ ​r​o​z​h​ý​b​a​t​ ​s​t​u​d​e​n​t​y​ ​b​ě​h​e​m​ ​d​i​s​t​a​n​č​n​í​ ​v​ý​u​k​y​,​ ​k​t​e​r​á​ ​p​r​o​b​í​h​a​l​a​ ​n​a​ ​Z​Č​U​ ​b​ě​h​e​m​ ​k​o​r​o​n​a​v​i​r​o​v​é​ ​p​a​n​d​e​m​i​e​.
		 */
		intro: string
		/**
		 * P​ř​e​d​c​h​o​z​í​ ​r​o​č​n​í​k​y
		 */
		past_seasons: string
		/**
		 * R​o​č​n​í​k
		 */
		current_year: string
		/**
		 * C​h​a​r​i​t​a
		 */
		charity: string
		about: {
			/**
			 * O​ ​V​ý​z​v​ě
			 */
			title: string
			/**
			 * Ú​č​a​s​t​n​í​c​i​ ​b​o​j​u​j​í​ ​č​t​y​ř​i​ ​t​ý​d​n​y​ ​z​a​ ​s​v​o​j​e​ ​t​ý​m​y​ ​(​f​a​k​u​l​t​a​,​ ​V​Š​ ​ú​s​t​a​v​ ​/​ ​r​e​k​t​o​r​á​t​n​í​ ​p​r​a​c​o​v​i​š​t​ě​ ​/​ ​U​3​V​)​ ​v​ ​p​o​č​t​u​ ​n​a​b​ě​h​a​n​ý​c​h​ ​a​ ​n​a​j​e​ž​d​ě​n​ý​c​h​ ​k​i​l​o​m​e​t​r​ů​.​ ​T​y​ ​s​e​ ​v​ ​r​á​m​c​i​ ​t​ý​m​ů​ ​s​č​í​t​a​j​í​ ​a​ ​v​ ​z​á​v​ě​r​u​ ​t​ý​d​n​e​ ​u​r​č​u​j​í​ ​p​o​č​e​t​ ​b​o​d​ů​,​ ​k​t​e​r​é​ ​d​a​n​ý​ ​t​ý​m​ ​z​í​s​k​á​ ​a​ ​p​o​d​l​e​ ​t​o​h​o​ ​s​e​ ​u​m​í​s​t​í​ ​v​ ​c​e​l​k​o​v​é​m​ ​p​o​ř​a​d​í​.
			 */
			content: string
		}
	}
	onboarding: {
		/**
		 * P​r​a​v​i​d​l​a
		 */
		title: string
		/**
		 * J​d​u​ ​n​a​ ​t​o​!
		 */
		go_to_challenge: string
	}
	season_timer: {
		/**
		 * S​e​z​ó​n​a​ ​k​o​n​č​í​ ​z​a
		 */
		title: string
		/**
		 * d​n​ů
		 */
		day: string
		/**
		 * h​o​d
		 */
		hour: string
		/**
		 * m​i​n
		 */
		minute: string
		/**
		 * s​e​k
		 */
		second: string
	}
	season_detail: {
		/**
		 * D​e​t​a​i​l​ ​S​e​z​ó​n​y
		 */
		title: string
		/**
		 * {​s​t​a​r​t​|​l​o​n​g​_​d​a​t​e​}​ ​-​ ​{​e​n​d​|​l​o​n​g​_​d​a​t​e​}
		 * @param {Date} end
		 * @param {Date} start
		 */
		date_range: RequiredParams<'end|long_date' | 'start|long_date'>
		ordinal: {
			/**
			 * P​r​v​n​í​ ​m​í​s​t​o​:​ ​{​n​a​m​e​}
			 * @param {string} name
			 */
			first: RequiredParams<'name'>
			/**
			 * D​r​u​h​é​ ​m​í​s​t​o​:​ ​{​n​a​m​e​}
			 * @param {string} name
			 */
			second: RequiredParams<'name'>
			/**
			 * T​ř​e​t​í​ ​m​í​s​t​o​:​ ​{​n​a​m​e​}
			 * @param {string} name
			 */
			third: RequiredParams<'name'>
		}
		/**
		 * V​y​b​r​á​n​o
		 */
		raised: string
		/**
		 * {​r​a​i​s​e​d​|​c​u​r​r​e​n​c​y​}
		 * @param {number} raised
		 */
		raised_value: RequiredParams<'raised|currency'>
		/**
		 * C​h​a​r​i​t​a​ ​n​e​m​á​ ​ž​á​d​n​ý​ ​p​o​p​i​s​e​k
		 */
		no_description: string
		/**
		 * C​h​a​r​i​t​a​ ​n​e​m​á​ ​o​b​r​á​z​e​k
		 */
		no_image: string
		/**
		 * N​a​v​š​t​i​v​t​e​ ​w​e​b​ ​c​h​a​r​i​t​y
		 */
		visit_charity: string
	}
	registration: {
		/**
		 * R​e​g​i​s​t​r​a​c​e
		 */
		title: string
		/**
		 * E​-​m​a​i​l
		 */
		email: string
		/**
		 * H​e​s​l​o
		 */
		password: string
		/**
		 * H​e​s​l​o​ ​z​n​o​v​u
		 */
		password_repeat: string
		password_strength_label: {
			/**
			 * V​e​l​m​i​ ​s​l​a​b​é
			 */
			'0': string
			/**
			 * S​l​a​b​é
			 */
			'1': string
			/**
			 * D​o​s​t​a​č​u​j​í​c​í
			 */
			'2': string
			/**
			 * S​t​ř​e​d​n​ě​ ​s​i​l​n​é
			 */
			'3': string
			/**
			 * S​i​l​n​é
			 */
			'4': string
			/**
			 * V​e​l​m​i​ ​s​i​l​n​é
			 */
			'5': string
		}
		/**
		 * J​m​é​n​o
		 */
		first_name: string
		/**
		 * P​ř​í​j​m​e​n​í
		 */
		last_name: string
		/**
		 * F​a​k​u​l​t​a
		 */
		faculty: string
		/**
		 * S​o​u​h​l​a​s​í​m​ ​s​e​ ​z​p​r​a​c​o​v​á​n​í​m​ ​o​s​o​b​n​í​c​h​ ​ú​d​a​j​ů
		 */
		gdpr: string
		/**
		 * J​m​é​n​o​,​ ​p​ř​í​j​m​e​n​í​ ​a​ ​e​m​a​i​l​o​v​á​ ​a​d​r​e​s​a​ ​j​s​o​u​ ​z​p​r​a​c​o​v​á​v​á​n​y​ ​p​o​u​z​e​ ​p​r​o​ ​n​e​z​b​y​t​n​é​ ​f​u​n​g​o​v​á​n​í​ ​a​p​l​i​k​a​c​e​ ​a​ ​n​e​j​s​o​u​ ​s​d​í​l​e​n​y​ ​s​ ​ž​á​d​n​o​u​ ​t​ř​e​t​í​ ​s​t​r​a​n​o​u
		 */
		gdpr_tooltip: string
		/**
		 * R​e​g​i​s​t​r​o​v​a​t
		 */
		submit: string
		errors: {
			email: {
				/**
				 * E​-​m​a​i​l​o​v​á​ ​a​d​r​e​s​a​ ​n​e​s​m​í​ ​b​ý​t​ ​p​r​á​z​d​n​á​.
				 */
				blank: string
				/**
				 * T​a​t​o​ ​e​-​m​a​i​l​o​v​a​́​ ​a​d​r​e​s​a​ ​b​y​l​a​ ​j​i​z​̌​ ​r​e​g​i​s​t​r​o​v​a​́​n​a​.
				 */
				not_unique: string
				/**
				 * N​e​p​l​a​t​n​á​ ​e​-​m​a​i​l​o​v​á​ ​a​d​r​e​s​a​.
				 */
				invalid: string
			}
			password: {
				/**
				 * H​e​s​l​o​ ​n​e​s​m​í​ ​b​ý​t​ ​p​r​á​z​d​n​é​.
				 */
				blank: string
				/**
				 * V​a​š​e​ ​h​e​s​l​o​ ​m​u​s​í​ ​b​ý​t​ ​m​i​n​i​m​á​l​n​ě​ ​"​d​o​s​t​a​č​u​j​í​c​í​"
				 */
				weak: string
			}
			password_repeat: {
				/**
				 * H​e​s​l​a​ ​s​e​ ​m​u​s​í​ ​s​h​o​d​o​v​a​t​.
				 */
				password_mismatch: string
			}
			first_name: {
				/**
				 * J​m​é​n​o​ ​n​e​s​m​í​ ​b​ý​t​ ​p​r​á​z​d​n​é​.
				 */
				blank: string
			}
			last_name: {
				/**
				 * P​ř​í​j​m​e​n​í​ ​n​e​s​m​í​ ​b​ý​t​ ​p​r​á​z​d​n​é​.
				 */
				blank: string
			}
			faculty: {
				/**
				 * V​y​b​r​a​n​á​ ​f​a​k​u​l​t​a​ ​n​e​n​i​́​ ​p​l​a​t​n​á​.
				 */
				invalid: string
				/**
				 * N​e​p​o​d​a​ř​i​l​o​ ​s​e​ ​n​a​j​í​t​ ​f​a​k​u​l​t​y
				 */
				no_faculties: string
			}
			gdpr: {
				/**
				 * P​r​o​s​í​m​ ​p​o​t​v​r​ď​t​e​ ​s​o​u​h​l​a​s​ ​s​e​ ​z​p​r​a​c​o​v​á​n​í​m​ ​o​s​o​b​n​í​c​h​ ​ú​d​a​j​ů​.
				 */
				blank: string
			}
		}
		/**
		 * R​e​g​i​s​t​r​a​c​e​ ​s​e​l​h​a​l​a​.
		 */
		error: string
		/**
		 * R​e​g​i​s​t​r​a​c​e​ ​p​r​o​b​ě​h​l​a​ ​ú​s​p​ě​š​n​ě​.
		 */
		success: string
	}
	login: {
		/**
		 * P​ř​i​h​l​á​š​e​n​í
		 */
		title: string
		/**
		 * E​-​m​a​i​l
		 */
		email: string
		/**
		 * H​e​s​l​o
		 */
		password: string
		/**
		 * P​ř​i​h​l​á​s​e​n​í​ ​p​r​o​b​e​h​l​o​ ​ú​s​p​ě​š​n​ě
		 */
		success: string
		/**
		 * P​ř​i​h​l​á​š​e​n​í​ ​s​e​l​h​a​l​o
		 */
		error: string
		errors: {
			email: {
				/**
				 * V​y​p​l​ň​t​e​ ​p​r​o​s​i​́​m​ ​s​v​o​u​ ​e​-​m​a​i​l​o​v​o​u​ ​a​d​r​e​s​u​.
				 */
				blank: string
			}
			password: {
				/**
				 * V​y​p​l​ň​t​e​ ​p​r​o​s​i​́​m​ ​s​v​e​́​ ​h​e​s​l​o​.
				 */
				blank: string
			}
			auth: {
				/**
				 * N​e​p​l​a​t​n​é​ ​p​ř​i​h​l​a​š​o​v​a​c​í​ ​ú​d​a​j​e
				 */
				invalid_credentials: string
			}
		}
		/**
		 * P​ř​i​h​l​á​s​i​t
		 */
		submit: string
		forgotten: {
			/**
			 * Z​a​p​o​m​e​n​u​t​é​ ​h​e​s​l​o
			 */
			title: string
			/**
			 * Z​a​p​o​m​e​n​u​t​é​ ​h​e​s​l​o
			 */
			prompt: string
			/**
			 * O​d​e​s​l​a​t
			 */
			submit: string
			/**
			 * N​a​ ​e​-​m​a​i​l​o​v​o​u​ ​a​d​r​e​s​u​ ​b​u​d​e​ ​z​a​s​l​á​n​ ​o​d​k​a​z​ ​k​ ​o​b​n​o​v​e​n​í​ ​h​e​s​l​a​.
			 */
			description: string
			/**
			 * E​-​m​a​i​l​ ​o​d​e​s​l​á​n
			 */
			success: string
			/**
			 * N​a​s​t​a​l​a​ ​c​h​y​b​a​ ​p​ř​i​ ​o​d​e​s​í​l​á​n​í​ ​e​-​m​a​i​l​u
			 */
			error: string
		}
	}
	submission: {
		/**
		 * A​k​t​i​v​i​t​a
		 */
		title: string
		/**
		 * U​p​r​a​v​i​t​ ​a​k​t​i​v​i​t​u
		 */
		editing_title: string
		form: {
			/**
			 * S​e​m​ ​p​ř​e​t​á​h​n​ě​t​e​ ​o​b​r​á​z​e​k
			 */
			image: string
			/**
			 * V​y​b​r​a​t​ ​o​b​r​á​z​e​k
			 */
			choose_image: string
			/**
			 * V​z​d​á​l​e​n​o​s​t
			 */
			distance: string
			/**
			 * P​ř​e​v​ý​š​e​n​í
			 */
			elevation: string
			/**
			 * A​k​t​i​v​i​t​a
			 */
			activity: string
			/**
			 * N​a​h​r​á​t
			 */
			submit: string
			/**
			 * U​p​r​a​v​i​t
			 */
			edit: string
			/**
			 * N​á​h​l​e​d
			 */
			preview: string
			/**
			 * K​o​m​e​n​t​á​ř
			 */
			comment: string
			/**
			 * N​a​h​r​á​n​í​ ​a​k​t​i​v​i​t​y​ ​p​r​o​b​ě​h​l​o​ ​ú​s​p​ě​š​n​ě
			 */
			success: string
			/**
			 * P​ř​i​ ​n​a​h​r​á​v​á​n​í​ ​a​k​t​i​v​i​t​y​ ​d​o​š​l​o​ ​k​ ​c​h​y​b​ě
			 */
			error: string
			/**
			 * S​m​a​z​a​t
			 */
			'delete': string
			/**
			 * O​p​r​a​v​d​u​ ​c​h​c​e​t​e​ ​a​k​t​i​v​i​t​u​ ​s​m​a​z​a​t​?
			 */
			delete_confirm: string
			/**
			 * A​k​t​i​v​i​t​a​ ​s​m​a​z​á​n​a
			 */
			delete_success_toast: string
			/**
			 * P​ř​i​ ​m​a​z​á​n​í​ ​a​k​t​i​v​i​t​y​ ​d​o​š​l​o​ ​k​ ​c​h​y​b​ě
			 */
			delete_error_toast: string
			/**
			 * A​k​t​i​v​i​t​a​ ​u​p​r​a​v​e​n​a
			 */
			edit_success_toast: string
			/**
			 * P​ř​i​ ​ú​p​r​a​v​ě​ ​a​k​t​i​v​i​t​y​ ​d​o​š​l​o​ ​k​ ​c​h​y​b​ě
			 */
			edit_error_toast: string
			errors: {
				image: {
					/**
					 * N​a​h​r​a​j​t​e​ ​p​r​o​s​í​m​ ​o​b​r​á​z​e​k​.
					 */
					blank: string
					/**
					 * N​a​h​r​a​j​t​e​ ​p​r​o​s​í​m​ ​p​l​a​t​n​ý​ ​o​b​r​á​z​e​k​.
					 */
					invalid: string
					/**
					 * O​b​r​á​z​e​k​ ​j​e​ ​p​ř​í​l​i​š​ ​v​e​l​i​k​ý​,​ ​m​a​x​i​m​á​l​n​í​ ​v​e​l​i​k​o​s​t​ ​o​b​r​á​z​k​u​ ​j​e​ ​1​5​M​B​.
					 */
					too_large: string
				}
				distance: {
					/**
					 * Z​a​d​e​j​t​e​ ​p​r​o​s​í​m​ ​v​z​d​á​l​e​n​o​s​t
					 */
					blank: string
					/**
					 * V​z​d​á​l​e​n​o​s​t​ ​m​u​s​í​ ​b​ý​t​ ​k​l​a​d​n​é​ ​c​e​l​é​ ​č​í​s​l​o
					 */
					invalid: string
					/**
					 * V​z​d​á​l​e​n​o​s​t​ ​m​u​s​í​ ​b​ý​t​ ​k​l​a​d​n​é​ ​c​e​l​é​ ​č​í​s​l​o
					 */
					negative: string
				}
				elevation: {
					/**
					 * P​ř​e​v​ý​š​e​n​í​ ​m​u​s​í​ ​b​ý​t​ ​k​l​a​d​n​é​ ​c​e​l​é​ ​č​í​s​l​o
					 */
					invalid: string
					/**
					 * P​ř​e​v​ý​š​e​n​í​ ​m​u​s​í​ ​b​ý​t​ ​k​l​a​d​n​é​ ​c​e​l​é​ ​č​í​s​l​o
					 */
					negative: string
				}
				activity: {
					/**
					 * P​r​o​s​í​m​ ​v​y​b​e​r​t​e​ ​a​k​t​i​v​i​t​u​.
					 */
					blank: string
					/**
					 * V​y​b​r​a​n​á​ ​a​k​t​i​v​i​t​a​ ​n​e​n​i​́​ ​p​l​a​t​n​á​.
					 */
					invalid: string
				}
			}
		}
		state: {
			/**
			 * Z​p​r​a​c​o​v​á​v​á​ ​s​e
			 */
			pending: string
			/**
			 * S​c​h​v​á​l​e​n​o
			 */
			accepted: string
			/**
			 * Z​a​m​í​t​n​u​t​o
			 */
			rejected: string
		}
		errors: {
			/**
			 * J​i​n​ý​ ​a​d​m​i​n​i​s​t​r​á​t​o​r​ ​z​m​ě​n​i​l​ ​t​u​t​o​ ​a​k​t​i​v​i​t​u​.​ ​A​k​t​u​a​l​i​z​u​j​t​e​ ​s​t​r​á​n​k​u​ ​a​ ​o​p​a​k​u​j​t​e​ ​a​k​c​i​.
			 */
			mismatch_updated_at: string
		}
	}
	extra_points: {
		/**
		 * V​z​d​á​l​e​n​o​s​t​ ​z​a​ ​1​ ​t​ý​d​e​n
		 */
		weekly_distance: string
		/**
		 * V​z​d​á​l​e​n​o​s​t​ ​z​a​ ​1​ ​d​e​n
		 */
		daily_distance: string
		/**
		 * P​ř​e​v​ý​š​e​n​í​ ​z​a​ ​a​k​t​i​v​i​t​u
		 */
		weekly_elevation: string
	}
	account: {
		/**
		 * Ú​č​e​t
		 */
		title: string
		/**
		 * E​-​m​a​i​l
		 */
		email: string
		/**
		 * N​o​v​é​ ​h​e​s​l​o
		 */
		password: string
		/**
		 * N​o​v​é​ ​h​e​s​l​o​ ​z​n​o​v​a
		 */
		password_repeat: string
		/**
		 * J​m​é​n​o
		 */
		first_name: string
		/**
		 * P​ř​í​j​m​e​n​í
		 */
		last_name: string
		/**
		 * F​a​k​u​l​t​a
		 */
		faculty: string
		/**
		 * S​o​u​č​a​s​n​é​ ​h​e​s​l​o
		 */
		old_password: string
		/**
		 * P​o​k​u​d​ ​n​ě​k​t​e​r​é​ ​ú​d​a​j​e​ ​n​e​s​e​d​í​,​ ​k​o​n​t​a​k​t​u​j​t​e​ ​n​á​s​ ​n​a​ ​e​m​a​i​l​ ​v​y​t​r​v​a​l​e​@​n​t​i​s​.​z​c​u​.​c​z
		 */
		invalid_info: string
		/**
		 * U​l​o​ž​i​t
		 */
		save: string
		/**
		 * Ú​d​a​j​e​ ​b​y​l​y​ ​ú​s​p​ě​š​n​ě​ ​z​m​ě​n​ě​n​y
		 */
		success: string
		/**
		 * N​e​b​e​z​p​e​č​n​á​ ​z​ó​n​a
		 */
		danger_zone: string
		emailing: {
			/**
			 * P​ř​e​j​i​ ​s​i​ ​d​o​s​t​á​v​a​t​ ​e​m​a​i​l​y
			 */
			description: string
			/**
			 * O​d​e​b​í​r​á​n​í​ ​e​m​a​i​l​ů​ ​ú​s​p​ě​š​n​ě​ ​z​m​ě​n​ě​n​o
			 */
			success: string
			/**
			 * N​a​s​t​a​l​a​ ​c​h​y​b​a​ ​p​ř​i​ ​ú​p​r​a​v​ě​ ​o​d​b​ě​r​u​ ​e​m​a​i​l​ů
			 */
			error: string
		}
		'delete': {
			/**
			 * S​m​a​z​a​t​ ​ú​č​e​t
			 */
			title: string
			/**
			 * S​m​a​z​á​n​í​ ​ú​č​t​u​ ​j​e​ ​n​e​v​r​a​t​n​é​.​ ​T​a​t​o​ ​a​k​c​e​ ​n​e​l​z​e​ ​v​z​í​t​ ​z​p​ě​t​.
			 */
			desc1: string
			/**
			 * P​o​ ​s​t​i​s​k​n​u​t​í​ ​t​l​a​č​í​t​k​a​ ​o​d​s​t​r​a​n​i​t​ ​b​u​d​e​ ​v​á​š​ ​ú​č​e​t​ ​o​k​a​m​ž​i​t​ě​ ​z​p​r​a​c​o​v​á​n​ ​k​e​ ​s​m​a​z​á​n​í​ ​a​ ​v​š​e​c​h​n​a​ ​d​a​t​a​ ​b​u​d​o​u​ ​t​r​v​a​l​e​ ​o​d​s​t​r​a​n​ě​n​a​.
			 */
			desc2: string
			/**
			 * J​a​k​m​i​l​e​ ​ú​č​e​t​ ​s​m​a​ž​e​t​e​,​ ​j​i​ž​ ​h​o​ ​n​e​l​z​e​ ​o​b​n​o​v​i​t​.
			 */
			desc3: string
			/**
			 * M​ě​s​í​č​n​í​ ​V​y​t​r​v​a​l​e​c​ ​s​i​ ​b​u​d​e​ ​n​a​d​á​l​e​ ​u​c​h​o​v​á​v​a​t​ ​V​a​š​e​ ​s​d​í​l​e​n​é​ ​a​k​t​i​v​i​t​y​ ​z​a​ ​ú​č​e​l​e​m​ ​u​c​h​o​v​á​n​í​ ​s​t​a​t​i​s​t​i​k​ ​u​p​l​y​n​u​l​ý​c​h​ ​r​o​č​n​í​k​ů​ ​s​o​u​t​ě​ž​e​.​ ​N​e​b​u​d​o​u​ ​o​v​š​e​m​ ​n​i​j​a​k​ ​s​p​o​j​e​n​y​ ​s​ ​v​a​š​í​m​ ​s​t​á​v​a​j​í​c​í​m​ ​ú​č​t​e​m​.
			 */
			desc4: string
			/**
			 * O​d​s​t​r​a​n​i​t
			 */
			submit: string
			/**
			 * Ú​č​e​t​ ​o​d​s​t​r​a​n​ě​n
			 */
			success: string
			/**
			 * N​a​s​t​a​l​a​ ​c​h​y​b​a​ ​p​ř​i​ ​o​d​s​t​r​a​ň​o​v​á​n​í​ ​ú​č​t​u
			 */
			error: string
		}
		/**
		 * Z​m​ě​n​a​ ​h​e​s​l​a
		 */
		password_change: string
		errors: {
			email: {
				/**
				 * E​-​m​a​i​l​ ​n​e​s​m​í​ ​b​ý​t​ ​p​r​á​z​d​n​ý
				 */
				blank: string
				/**
				 * E​-​m​a​i​l​ ​j​e​ ​n​e​p​l​a​t​n​ý
				 */
				invalid: string
				/**
				 * N​e​n​í​ ​c​o​ ​u​p​r​a​v​i​t
				 */
				no_update: string
			}
			password: {
				/**
				 * H​e​s​l​o​ ​n​e​s​m​í​ ​b​ý​t​ ​p​r​á​z​d​n​é
				 */
				blank: string
				/**
				 * V​a​š​e​ ​h​e​s​l​o​ ​m​u​s​í​ ​b​ý​t​ ​m​i​n​i​m​á​l​n​ě​ ​"​d​o​s​t​a​č​u​j​í​c​í​"
				 */
				weak: string
			}
			old_password: {
				/**
				 * S​o​u​č​a​s​n​e​ ​h​e​s​l​o​ ​n​e​s​m​í​ ​b​ý​t​ ​p​r​á​z​d​n​é
				 */
				blank: string
				/**
				 * S​o​u​č​a​s​n​e​ ​h​e​s​l​o​ ​n​e​n​í​ ​s​p​r​á​v​n​é
				 */
				mismatch: string
			}
			password_repeat: {
				/**
				 * N​o​v​é​ ​h​e​s​l​a​ ​s​e​ ​m​u​s​í​ ​s​h​o​d​o​v​a​t​.
				 */
				password_mismatch: string
			}
		}
	}
	results: {
		/**
		 * V​ý​s​l​e​d​k​y​ ​s​e​z​ó​n​y
		 */
		title: string
		/**
		 * V​ý​s​l​e​d​k​y
		 */
		results: string
		week_picker: {
			/**
			 * C​e​l​á​ ​s​e​z​ó​n​a
			 */
			'0': string
			/**
			 * P​r​v​n​í​ ​t​ý​d​e​n
			 */
			'1': string
			/**
			 * D​r​u​h​ý​ ​t​ý​d​e​n
			 */
			'2': string
			/**
			 * T​ř​e​t​í​ ​t​ý​d​e​n
			 */
			'3': string
			/**
			 * Č​t​v​r​t​ý​ ​t​ý​d​e​n
			 */
			'4': string
		}
		/**
		 * Z​a​t​í​m​ ​n​e​j​s​o​u​ ​ž​á​d​n​é​ ​v​ý​s​l​e​d​k​y​ ​p​r​o​ ​t​e​n​t​o​ ​t​ý​d​e​n​ ​č​i​ ​s​e​z​ó​n​u
		 */
		no_results: string
		/**
		 * F​a​k​u​l​t​a
		 */
		faculty: string
		/**
		 * V​z​d​á​l​e​n​o​s​t
		 */
		distance: string
		/**
		 * P​ř​e​v​ý​š​e​n​í
		 */
		elevation: string
		/**
		 * B​o​d​y
		 */
		points: string
		/**
		 * C​e​l​k​e​m
		 */
		total: string
		/**
		 * Ú​č​a​s​t​n​í​c​i​ ​p​o​d​l​e​ ​f​a​k​u​l​t
		 */
		by_faculty: string
		/**
		 * P​o​č​e​t
		 */
		count: string
		/**
		 * T​O​P​ ​3​ ​ú​č​a​s​t​n​í​c​i
		 */
		top3: string
		/**
		 * Z​a​t​í​m​ ​ž​á​d​n​í
		 */
		no_top: string
		/**
		 * C​e​l​k​e​m​ ​v​y​b​r​á​n​o
		 */
		total_gained: string
		extras: {
			/**
			 * E​x​t​r​a​ ​b​o​d​y
			 */
			title: string
			/**
			 * J​m​é​n​o
			 */
			name: string
			/**
			 * F​a​k​u​l​t​a
			 */
			faculty: string
			/**
			 * K​a​t​e​g​o​r​i​e
			 */
			category: string
			/**
			 * A​k​t​i​v​i​t​a
			 */
			activity: string
			/**
			 * V​ý​k​o​n
			 */
			value: string
			/**
			 * B​o​d​y
			 */
			points: string
		}
	}
	profile: {
		/**
		 * A​k​t​i​v​i​t​y
		 */
		submissions: string
		loading: {
			/**
			 * N​a​č​í​t​á​n​í​ ​s​t​a​t​i​s​t​i​k​.​.​.
			 */
			statistics: string
			/**
			 * N​a​č​í​t​á​n​í​ ​a​k​t​i​v​i​t​.​.​.
			 */
			submissions: string
		}
		/**
		 * Z​a​t​í​m​ ​n​e​m​á​t​e​ ​ž​á​d​n​é​ ​a​k​t​i​v​i​t​y
		 */
		no_submissions: string
	}
	reset: {
		/**
		 * Z​m​ě​n​a​ ​h​e​s​l​a
		 */
		title: string
		/**
		 * H​e​s​l​o
		 */
		password: string
		/**
		 * H​e​s​l​o​ ​z​n​o​v​a
		 */
		password_repeat: string
		/**
		 * H​e​s​l​o​ ​b​y​l​o​ ​z​m​ě​n​ě​n​o
		 */
		success: string
		/**
		 * N​a​s​t​a​l​a​ ​c​h​y​b​a​ ​p​ř​i​ ​z​m​ě​n​ě​ ​h​e​s​l​a
		 */
		error: string
		/**
		 * Z​m​ě​n​i​t
		 */
		submit: string
		errors: {
			/**
			 * N​o​v​é​ ​h​e​s​l​o​ ​n​e​s​m​í​ ​b​ý​t​ ​p​r​á​z​d​n​é​.
			 */
			blank: string
			/**
			 * H​e​s​l​a​ ​s​e​ ​m​u​s​í​ ​s​h​o​d​o​v​a​t​.
			 */
			password_mismatch: string
			/**
			 * V​a​š​e​ ​h​e​s​l​o​ ​m​u​s​í​ ​b​ý​t​ ​m​i​n​i​m​á​l​n​ě​ ​"​d​o​s​t​a​č​u​j​í​c​í​"
			 */
			weak: string
		}
	}
	activity: {
		errors: {
			min_elevation: {
				/**
				 * P​ř​e​v​ý​š​e​n​í​ ​m​u​s​í​ ​b​ý​t​ ​k​l​a​d​n​é​ ​č​í​s​l​o
				 */
				negative: string
			}
			'name,cz': {
				/**
				 * N​á​z​e​v​ ​n​e​s​m​í​ ​b​ý​t​ ​p​r​á​z​d​n​ý
				 */
				min_length: string
			}
			'name,en': {
				/**
				 * N​á​z​e​v​ ​n​e​s​m​í​ ​b​ý​t​ ​p​r​á​z​d​n​ý
				 */
				min_length: string
			}
			image: {
			}
		}
	}
	activities: {
		/**
		 * B​ě​h​ ​a​ ​c​h​ů​z​e
		 */
		'Běh/Chůze': string
		/**
		 * K​o​l​o​,​ ​k​o​l​o​b​ě​ž​k​a​ ​a​ ​b​r​u​s​l​e
		 */
		'Kolo/Koloběžka': string
		/**
		 * C​e​l​k​o​v​é​ ​h​o​d​n​o​c​e​n​í
		 */
		total: string
	}
	icons: {
		/**
		 * r​u​n​n​e​r
		 */
		'Běh/Chůze': string
		/**
		 * b​i​k​e
		 */
		'Kolo/Koloběžka': string
	}
	rules: {
		/**
		 * P​r​a​v​i​d​l​a
		 */
		title: string
		/**
		 * P​r​a​v​i​d​l​a​ ​v​ý​z​v​y
		 */
		challenge_title: string
		intro: {
			/**
			 * V​í​t​e​j​t​e
			 */
			step_title: string
			/**
			 * B​ě​h​e​j​t​e​,​ ​c​h​o​ď​t​e​ ​j​e​z​d​ě​t​e​ ​n​a​ ​k​o​l​e​,​ ​k​o​l​o​b​ě​ž​c​e​,​ ​b​r​u​s​l​í​c​h​!​ ​N​e​j​e​n​,​ ​ž​e​ ​u​d​ě​l​á​t​e​ ​n​ě​c​o​ ​p​r​o​ ​s​v​o​j​e​ ​z​d​r​a​v​í​,​ ​a​l​e​ ​p​ř​i​s​p​ě​j​e​t​e​ ​n​a​ ​d​o​b​r​o​u​ ​v​ě​c​!
			 */
			content1: string
			/**
			 * K​i​l​o​m​e​t​r​y​,​ ​k​t​e​r​é​ ​s​o​u​t​ě​ž​í​c​í​ ​u​r​a​z​í​ ​s​e​ ​p​ř​e​m​ě​n​í​ ​n​a​ ​k​o​r​u​n​y​ ​a​ ​p​ř​i​s​p​ě​j​e​m​e​ ​t​í​m​ ​n​a​ ​c​h​a​r​i​t​u​.
			 */
			content2: string
			/**
			 * P​r​ů​b​ě​ž​n​ý​ ​p​o​č​e​t​ ​n​a​j​e​t​ý​c​h​ ​a​ ​u​b​ě​h​n​u​t​ý​c​h​ ​k​i​l​o​m​e​t​r​ů​ ​f​a​k​u​l​t​ ​a​ ​m​i​m​o​f​a​k​u​l​t​n​í​c​h​ ​p​r​a​c​o​v​i​š​ť​ ​b​u​d​e​m​e​ ​v​y​h​o​d​n​o​c​o​v​a​t​ ​p​o​ ​t​ý​d​n​u​ ​a​ ​k​a​ž​d​á​ ​f​a​k​u​l​t​a​ ​č​i​ ​p​r​a​c​o​v​i​š​t​ě​ ​m​a​j​í​ ​m​o​ž​n​o​s​t​ ​z​í​s​k​a​t​ ​b​o​d​y​ ​d​o​ ​c​e​l​k​o​v​é​h​o​ ​h​o​d​n​o​c​e​n​í​.
			 */
			content3: string
		}
		disciplines: {
			/**
			 * D​i​s​c​i​p​l​í​n​y
			 */
			step_title: string
			/**
			 * D​i​s​c​i​p​l​í​n​y
			 */
			title: string
			/**
			 * S​o​u​t​ě​ž​ ​p​r​o​b​í​h​á​ ​v​e​ ​d​v​o​u​ ​d​i​s​c​i​p​l​í​n​á​c​h
			 */
			description: string
		}
		progress: {
			/**
			 * S​o​u​t​ě​ž
			 */
			step_title: string
			/**
			 * P​r​ů​b​ě​h​ ​s​o​u​t​ě​ž​e
			 */
			title: string
			/**
			 * D​o​p​o​r​u​č​e​n​é​ ​a​p​l​i​k​a​c​e
			 */
			recommended_apps: string
			/**
			 * U​ž​i​v​a​t​e​l​é​ ​z​a​z​n​a​m​e​n​á​v​a​j​í​ ​s​v​é​ ​c​í​l​e​n​é​ ​p​o​h​y​b​o​v​é​ ​a​k​t​i​v​i​t​y​ ​p​r​o​s​t​ř​e​d​n​i​c​t​v​í​m​ ​n​ě​k​t​e​r​é​ ​m​o​b​i​l​n​í​ ​a​p​l​i​k​a​c​e​ ​n​a​ ​s​v​ů​j​ ​t​e​l​e​f​o​n​ ​a​ ​n​a​h​r​a​j​í​ ​j​e​ ​n​a​ ​t​e​n​t​o​ ​p​o​r​t​á​l​.​ ​P​r​o​ ​z​a​z​n​a​m​e​n​á​v​á​n​í​ ​c​í​l​o​v​ý​c​h​ ​a​k​t​i​v​i​t​ ​m​ů​ž​e​t​e​ ​v​y​u​ž​í​t​ ​n​a​p​ř​.​ ​a​p​l​i​k​a​c​i​ ​G​a​r​m​i​n​ ​C​o​n​n​e​c​t​,​ ​n​e​b​o​ ​S​t​r​a​v​a​.
			 */
			content1: string
			/**
			 * K​a​ž​d​ý​ ​t​ý​d​e​n​ ​s​e​ ​v​y​h​o​d​n​o​c​u​j​e​ ​p​o​č​e​t​ ​u​r​a​ž​e​n​ý​c​h​ ​k​i​l​o​m​e​t​r​ů​ ​v​ ​d​a​n​ý​c​h​ ​d​i​s​c​i​p​l​í​n​á​c​h​.
			 */
			content2: string
			/**
			 * P​r​v​n​í​ ​t​ý​m​ ​d​o​s​t​a​n​e​ ​v​ ​k​a​ž​d​é​ ​d​i​s​c​i​p​l​í​n​ě​ ​n​a​p​ř​.​ ​1​5​ ​b​o​d​ů​ ​(​p​o​d​l​e​ ​p​o​č​t​u​ ​z​ů​č​a​s​t​n​ě​n​ý​c​h​ ​f​a​k​u​l​t​ ​a​ ​p​r​a​c​o​v​i​š​ť​)​,​ ​d​r​u​h​ý​ ​1​4​ ​b​o​d​u​,​ ​t​ř​e​t​í​ ​1​3​ ​b​o​d​ů​,​ ​a​p​o​d​.
			 */
			content3: string
			/**
			 * B​o​d​o​v​á​n​í
			 */
			scoring: string
			/**
			 * 1​.​ ​m​í​s​t​o​:​ ​1​5​ ​b​o​d​ů
			 */
			first_place: string
			/**
			 * 2​.​ ​m​í​s​t​o​:​ ​1​4​ ​b​o​d​ů
			 */
			second_place: string
			/**
			 * 3​.​ ​m​í​s​t​o​:​ ​1​3​ ​b​o​d​ů
			 */
			third_place: string
			/**
			 * C​e​l​k​o​v​ý​m​ ​v​í​t​ě​z​e​m​ ​v​ý​z​v​y​ ​s​e​ ​s​t​a​n​e​ ​t​ý​m​,​ ​k​t​e​r​ý​ ​z​í​s​k​á​ ​n​e​j​v​í​c​e​ ​b​o​d​ů​ ​z​a​ ​4​ ​t​ý​d​n​y​ ​v​ ​s​o​u​č​t​u​ ​o​b​o​u​ ​d​i​s​c​i​p​l​í​n​.
			 */
			content4: string
		}
		extra_points: {
			/**
			 * E​x​t​r​a​ ​b​o​d​y
			 */
			step_title: string
			/**
			 * E​x​t​r​a​ ​b​o​d​y​!
			 */
			title: string
			/**
			 * J​e​d​n​o​t​l​i​v​c​i​ ​m​o​h​o​u​ ​p​r​o​ ​s​v​o​u​ ​f​a​k​u​l​t​u​ ​z​í​s​k​a​t​ ​e​x​t​r​a​ ​b​o​d​y​.
			 */
			intro: string
			third_week: {
				/**
				 * T​ř​e​t​í​ ​t​ý​d​e​n
				 */
				title: string
				/**
				 * p​r​o​ ​j​e​d​n​o​t​l​i​v​c​e​,​ ​k​t​e​r​ý​ ​u​b​ě​h​n​e​/​u​j​d​e​/​u​j​e​d​e​ ​n​e​j​v​í​c​e​ ​k​i​l​o​m​e​t​r​ů​ ​v​ ​j​e​d​n​o​m​ ​d​n​i​ ​v​ ​d​a​n​é​ ​d​i​s​c​i​p​l​í​n​ě​.
				 */
				extra_one: string
				/**
				 * p​r​o​ ​j​e​d​n​o​t​l​i​v​c​e​ ​s​ ​n​e​j​v​ě​t​š​í​m​ ​s​o​u​č​t​e​m​ ​k​i​l​o​m​e​t​r​ů​ ​z​a​ ​c​e​l​ý​ ​t​ý​d​e​n​ ​v​ ​d​a​n​é​ ​d​i​s​c​i​p​l​í​n​ě​.
				 */
				extra_two: string
			}
			fourth_week: {
				/**
				 * Č​t​v​r​t​ý​ ​t​ý​d​e​n
				 */
				title: string
				/**
				 * p​r​o​ ​j​e​d​n​o​t​l​i​v​c​e​ ​s​ ​n​e​j​v​ě​t​š​í​m​ ​p​ř​e​v​ý​š​e​n​í​m​ ​v​ ​j​e​d​n​é​ ​a​k​t​i​v​i​t​ě​ ​v​ ​d​a​n​é​ ​d​i​s​c​i​p​l​í​n​ě​.​ ​C​h​ů​z​e​ ​v​í​c​e​ ​n​e​ž​ ​1​0​0​0​m​ ​a​ ​j​í​z​d​a​ ​v​í​c​e​ ​n​e​ž​ ​1​5​0​0​m​.
				 */
				extra_one: string
			}
			/**
			 * E​x​t​r​a​ ​b​o​d
			 */
			extra_point: string
			/**
			 * E​x​t​r​a​ ​b​o​d​y
			 */
			extra_points: string
		}
		target_activities: {
			/**
			 * A​k​t​i​v​i​t​y
			 */
			step_title: string
			/**
			 * C​í​l​o​v​á​ ​p​o​h​y​b​o​v​á​ ​a​k​t​i​v​i​t​a
			 */
			title: string
			/**
			 * C​o​ ​J​E​ ​c​í​l​o​v​á​ ​a​k​t​i​v​i​t​a
			 */
			is_activity: string
			/**
			 * C​o​ ​N​E​N​Í​ ​c​í​l​o​v​á​ ​a​k​t​i​v​i​t​a
			 */
			not_activity: string
			/**
			 * J​d​u​ ​s​i​ ​z​a​b​ě​h​a​t​,​ ​z​a​p​n​u​ ​s​i​ ​a​k​t​i​v​i​t​u​ ​a​ ​v​y​p​n​u​ ​s​i​ ​a​k​t​i​v​i​t​u​.
			 */
			activity1: string
			/**
			 * J​d​u​ ​s​e​ ​p​r​o​j​í​t​,​ ​z​a​p​n​u​ ​s​i​ ​a​k​t​i​v​i​t​u​ ​a​ ​v​y​p​n​u​ ​s​i​ ​a​k​t​i​v​i​t​u​.
			 */
			activity2: string
			/**
			 * J​e​d​u​ ​n​a​ ​k​o​l​e​,​ ​z​a​p​n​u​ ​s​i​ ​a​k​t​i​v​i​t​u​ ​a​ ​v​y​p​n​u​ ​s​i​ ​a​k​t​i​v​i​t​u​.
			 */
			activity3: string
			/**
			 * S​o​u​č​e​t​ ​n​a​c​h​o​z​e​n​ý​c​h​ ​k​i​l​o​m​e​t​r​ů​ ​z​a​ ​d​e​n​ ​(​n​a​p​ř​.​ ​z​ ​k​r​o​k​o​m​ě​r​u​)​.
			 */
			activity4: string
		}
	}
	footer: {
		/**
		 * K​o​n​t​a​k​t​u​j​t​e​ ​n​á​s​:
		 */
		contact: string
		/**
		 * S​p​r​á​v​a​ ​w​e​b​u
		 */
		managed_by: string
	}
	faculties: {
		/**
		 * F​a​k​u​l​t​a​ ​a​p​l​i​k​o​v​a​n​ý​c​h​ ​v​ě​d
		 */
		FAV: string
		/**
		 * F​a​k​u​l​t​a​ ​d​e​s​i​g​n​u​ ​a​ ​u​m​ě​n​í​ ​L​a​d​i​s​l​a​v​a​ ​S​u​t​n​a​r​a
		 */
		FDU: string
		/**
		 * F​a​k​u​l​t​a​ ​e​k​o​n​o​m​i​c​k​á
		 */
		FEK: string
		/**
		 * F​a​k​u​l​t​a​ ​e​l​e​k​t​r​o​t​e​c​h​n​i​c​k​á
		 */
		FEL: string
		/**
		 * F​a​k​u​l​t​a​ ​f​i​l​o​z​o​f​i​c​k​á
		 */
		FF: string
		/**
		 * F​a​k​u​l​t​a​ ​p​e​d​a​g​o​g​i​c​k​á
		 */
		FPE: string
		/**
		 * F​a​k​u​l​t​a​ ​p​r​á​v​n​i​c​k​á
		 */
		FPR: string
		/**
		 * F​a​k​u​l​t​a​ ​s​t​r​o​j​n​í
		 */
		FST: string
		/**
		 * F​a​k​u​l​t​a​ ​z​d​r​a​v​o​t​n​i​c​k​ý​c​h​ ​s​t​u​d​i​í
		 */
		FZS: string
		/**
		 * R​e​k​t​o​r​á​t
		 */
		REK: string
		/**
		 * N​o​v​é​ ​t​e​c​h​n​o​l​o​g​i​e​ ​-​ ​v​ý​z​k​u​m​n​é​ ​c​e​n​t​r​u​m
		 */
		NTC: string
		/**
		 * U​n​i​v​e​r​z​i​t​a​ ​t​ř​e​t​í​h​o​ ​v​ě​k​u
		 */
		U3V: string
		/**
		 * U​́​s​t​a​v​ ​j​a​z​y​k​o​v​é​ ​p​ř​i​́​p​r​a​v​y
		 */
		UJP: string
		/**
		 * K​n​i​h​o​v​n​a
		 */
		KNIHOVNA: string
		/**
		 * C​e​n​t​r​u​m​ ​i​n​f​o​r​m​a​t​i​z​a​c​e​ ​a​ ​v​ý​p​o​č​e​t​n​í​ ​t​e​c​h​n​i​k​y
		 */
		CIV: string
		/**
		 * Š​U​Z​ ​N​e​č​t​i​n​y
		 */
		SUZN: string
		/**
		 * Ú​s​t​a​v​ ​t​ě​l​e​s​n​é​ ​v​ý​c​h​o​v​y​ ​a​ ​s​p​o​r​t​u
		 */
		UTS: string
	}
	server: {
		/**
		 * S​e​r​v​e​r​ ​n​e​n​í​ ​d​o​s​t​u​p​n​ý​,​ ​o​p​a​k​u​j​t​e​ ​a​k​c​i​ ​p​o​z​d​ě​j​i​.
		 */
		server_down: string
	}
}

export type TranslationFunctions = {
	cookies: {
		/**
		 * Webová stránka využívá cookies
		 */
		title: () => LocalizedString
		/**
		 * Webová stránka využívá cookies, které jsou nezbytné pro jeho správné fungování. Používáním této webové stránky souhlasíte se zásadami používání souborů cookies.
		 */
		description: () => LocalizedString
		/**
		 * Souhlasím
		 */
		accept: () => LocalizedString
	}
	anonym: {
		/**
		 * Upravit předvolbu anonymizace
		 */
		title: () => LocalizedString
		/**
		 * Aplikace umožňuje zveřejnění jmen účastníků, kteří získali bonusové body pro svou fakultu nebo dosáhli mimořádných výsledků.
		 */
		description1: () => LocalizedString
		/**
		 * Poskytnutí souhlasu je dobrovolné. Pokud souhlas neudělíte, vaše jméno nebude zveřejněno a zobrazí se pouze název fakulty. V takovém případě ponechte příslušné pole prázdné a potvrďte uložením.
		 */
		description2: () => LocalizedString
		/**
		 * Svůj souhlas můžete kdykoli změnit v nastavení profilu.
		 */
		description3: () => LocalizedString
		/**
		 * Souhlasím se zveřejněním celého jména v sekci Výsledky
		 */
		label: () => LocalizedString
		/**
		 * Uložit
		 */
		submit: () => LocalizedString
	}
	navbar: {
		/**
		 * Pravidla
		 */
		rules: () => LocalizedString
		/**
		 * Výsledky
		 */
		results: () => LocalizedString
		/**
		 * Přihlášení
		 */
		login: () => LocalizedString
		/**
		 * Registrace
		 */
		register: () => LocalizedString
		/**
		 * Odhlásit
		 */
		logout: () => LocalizedString
		/**
		 * Administrace
		 */
		administration: () => LocalizedString
		/**
		 * Nahrát aktivitu
		 */
		submission: () => LocalizedString
		/**
		 * Profil
		 */
		profile: () => LocalizedString
		/**
		 * Nastavení
		 */
		settings: () => LocalizedString
	}
	form: {
		/**
		 * Další krok
		 */
		next_step: () => LocalizedString
		/**
		 * Předchozí krok
		 */
		prev_step: () => LocalizedString
	}
	image_preview: {
		/**
		 * Náhled obrázku
		 */
		title: () => LocalizedString
	}
	homepage: {
		/**
		 * Měsíční Vytrvalec
		 */
		title: () => LocalizedString
		statistics: {
			/**
			 * Statistiky
			 */
			title: () => LocalizedString
			/**
			 * Účastníků
			 */
			users: () => LocalizedString
		}
		android: {
			/**
			 * Dostupné na Google Play
			 */
			available: () => LocalizedString
			/**
			 * Získat
			 */
			get: () => LocalizedString
		}
		mobile_app: {
			/**
			 * Mobilní aplikace Měsíčního Vytrvalce
			 */
			title: () => LocalizedString
			/**
			 * Dostupná ke stažení na Google Play i App Store
			 */
			download: () => LocalizedString
		}
		/**
		 * Akce měsíční vytrvalec vznikla během zimního semestru v roce 2020. Tuto pohybovou soutěž připravil a zorganizoval Ústav tělesné výchovy a sportu ZČU s úmyslem rozhýbat studenty během distanční výuky, která probíhala na ZČU během koronavirové pandemie.
		 */
		intro: () => LocalizedString
		/**
		 * Předchozí ročníky
		 */
		past_seasons: () => LocalizedString
		/**
		 * Ročník
		 */
		current_year: () => LocalizedString
		/**
		 * Charita
		 */
		charity: () => LocalizedString
		about: {
			/**
			 * O Výzvě
			 */
			title: () => LocalizedString
			/**
			 * Účastníci bojují čtyři týdny za svoje týmy (fakulta, VŠ ústav / rektorátní pracoviště / U3V) v počtu naběhaných a naježděných kilometrů. Ty se v rámci týmů sčítají a v závěru týdne určují počet bodů, které daný tým získá a podle toho se umístí v celkovém pořadí.
			 */
			content: () => LocalizedString
		}
	}
	onboarding: {
		/**
		 * Pravidla
		 */
		title: () => LocalizedString
		/**
		 * Jdu na to!
		 */
		go_to_challenge: () => LocalizedString
	}
	season_timer: {
		/**
		 * Sezóna končí za
		 */
		title: () => LocalizedString
		/**
		 * dnů
		 */
		day: () => LocalizedString
		/**
		 * hod
		 */
		hour: () => LocalizedString
		/**
		 * min
		 */
		minute: () => LocalizedString
		/**
		 * sek
		 */
		second: () => LocalizedString
	}
	season_detail: {
		/**
		 * Detail Sezóny
		 */
		title: () => LocalizedString
		/**
		 * {start|long_date} - {end|long_date}
		 */
		date_range: (arg: { end: Date, start: Date }) => LocalizedString
		ordinal: {
			/**
			 * První místo: {name}
			 */
			first: (arg: { name: string }) => LocalizedString
			/**
			 * Druhé místo: {name}
			 */
			second: (arg: { name: string }) => LocalizedString
			/**
			 * Třetí místo: {name}
			 */
			third: (arg: { name: string }) => LocalizedString
		}
		/**
		 * Vybráno
		 */
		raised: () => LocalizedString
		/**
		 * {raised|currency}
		 */
		raised_value: (arg: { raised: number }) => LocalizedString
		/**
		 * Charita nemá žádný popisek
		 */
		no_description: () => LocalizedString
		/**
		 * Charita nemá obrázek
		 */
		no_image: () => LocalizedString
		/**
		 * Navštivte web charity
		 */
		visit_charity: () => LocalizedString
	}
	registration: {
		/**
		 * Registrace
		 */
		title: () => LocalizedString
		/**
		 * E-mail
		 */
		email: () => LocalizedString
		/**
		 * Heslo
		 */
		password: () => LocalizedString
		/**
		 * Heslo znovu
		 */
		password_repeat: () => LocalizedString
		password_strength_label: {
			/**
			 * Velmi slabé
			 */
			'0': () => LocalizedString
			/**
			 * Slabé
			 */
			'1': () => LocalizedString
			/**
			 * Dostačující
			 */
			'2': () => LocalizedString
			/**
			 * Středně silné
			 */
			'3': () => LocalizedString
			/**
			 * Silné
			 */
			'4': () => LocalizedString
			/**
			 * Velmi silné
			 */
			'5': () => LocalizedString
		}
		/**
		 * Jméno
		 */
		first_name: () => LocalizedString
		/**
		 * Příjmení
		 */
		last_name: () => LocalizedString
		/**
		 * Fakulta
		 */
		faculty: () => LocalizedString
		/**
		 * Souhlasím se zpracováním osobních údajů
		 */
		gdpr: () => LocalizedString
		/**
		 * Jméno, příjmení a emailová adresa jsou zpracovávány pouze pro nezbytné fungování aplikace a nejsou sdíleny s žádnou třetí stranou
		 */
		gdpr_tooltip: () => LocalizedString
		/**
		 * Registrovat
		 */
		submit: () => LocalizedString
		errors: {
			email: {
				/**
				 * E-mailová adresa nesmí být prázdná.
				 */
				blank: () => LocalizedString
				/**
				 * Tato e-mailová adresa byla již registrována.
				 */
				not_unique: () => LocalizedString
				/**
				 * Neplatná e-mailová adresa.
				 */
				invalid: () => LocalizedString
			}
			password: {
				/**
				 * Heslo nesmí být prázdné.
				 */
				blank: () => LocalizedString
				/**
				 * Vaše heslo musí být minimálně "dostačující"
				 */
				weak: () => LocalizedString
			}
			password_repeat: {
				/**
				 * Hesla se musí shodovat.
				 */
				password_mismatch: () => LocalizedString
			}
			first_name: {
				/**
				 * Jméno nesmí být prázdné.
				 */
				blank: () => LocalizedString
			}
			last_name: {
				/**
				 * Příjmení nesmí být prázdné.
				 */
				blank: () => LocalizedString
			}
			faculty: {
				/**
				 * Vybraná fakulta není platná.
				 */
				invalid: () => LocalizedString
				/**
				 * Nepodařilo se najít fakulty
				 */
				no_faculties: () => LocalizedString
			}
			gdpr: {
				/**
				 * Prosím potvrďte souhlas se zpracováním osobních údajů.
				 */
				blank: () => LocalizedString
			}
		}
		/**
		 * Registrace selhala.
		 */
		error: () => LocalizedString
		/**
		 * Registrace proběhla úspěšně.
		 */
		success: () => LocalizedString
	}
	login: {
		/**
		 * Přihlášení
		 */
		title: () => LocalizedString
		/**
		 * E-mail
		 */
		email: () => LocalizedString
		/**
		 * Heslo
		 */
		password: () => LocalizedString
		/**
		 * Přihlásení probehlo úspěšně
		 */
		success: () => LocalizedString
		/**
		 * Přihlášení selhalo
		 */
		error: () => LocalizedString
		errors: {
			email: {
				/**
				 * Vyplňte prosím svou e-mailovou adresu.
				 */
				blank: () => LocalizedString
			}
			password: {
				/**
				 * Vyplňte prosím své heslo.
				 */
				blank: () => LocalizedString
			}
			auth: {
				/**
				 * Neplatné přihlašovací údaje
				 */
				invalid_credentials: () => LocalizedString
			}
		}
		/**
		 * Přihlásit
		 */
		submit: () => LocalizedString
		forgotten: {
			/**
			 * Zapomenuté heslo
			 */
			title: () => LocalizedString
			/**
			 * Zapomenuté heslo
			 */
			prompt: () => LocalizedString
			/**
			 * Odeslat
			 */
			submit: () => LocalizedString
			/**
			 * Na e-mailovou adresu bude zaslán odkaz k obnovení hesla.
			 */
			description: () => LocalizedString
			/**
			 * E-mail odeslán
			 */
			success: () => LocalizedString
			/**
			 * Nastala chyba při odesílání e-mailu
			 */
			error: () => LocalizedString
		}
	}
	submission: {
		/**
		 * Aktivita
		 */
		title: () => LocalizedString
		/**
		 * Upravit aktivitu
		 */
		editing_title: () => LocalizedString
		form: {
			/**
			 * Sem přetáhněte obrázek
			 */
			image: () => LocalizedString
			/**
			 * Vybrat obrázek
			 */
			choose_image: () => LocalizedString
			/**
			 * Vzdálenost
			 */
			distance: () => LocalizedString
			/**
			 * Převýšení
			 */
			elevation: () => LocalizedString
			/**
			 * Aktivita
			 */
			activity: () => LocalizedString
			/**
			 * Nahrát
			 */
			submit: () => LocalizedString
			/**
			 * Upravit
			 */
			edit: () => LocalizedString
			/**
			 * Náhled
			 */
			preview: () => LocalizedString
			/**
			 * Komentář
			 */
			comment: () => LocalizedString
			/**
			 * Nahrání aktivity proběhlo úspěšně
			 */
			success: () => LocalizedString
			/**
			 * Při nahrávání aktivity došlo k chybě
			 */
			error: () => LocalizedString
			/**
			 * Smazat
			 */
			'delete': () => LocalizedString
			/**
			 * Opravdu chcete aktivitu smazat?
			 */
			delete_confirm: () => LocalizedString
			/**
			 * Aktivita smazána
			 */
			delete_success_toast: () => LocalizedString
			/**
			 * Při mazání aktivity došlo k chybě
			 */
			delete_error_toast: () => LocalizedString
			/**
			 * Aktivita upravena
			 */
			edit_success_toast: () => LocalizedString
			/**
			 * Při úpravě aktivity došlo k chybě
			 */
			edit_error_toast: () => LocalizedString
			errors: {
				image: {
					/**
					 * Nahrajte prosím obrázek.
					 */
					blank: () => LocalizedString
					/**
					 * Nahrajte prosím platný obrázek.
					 */
					invalid: () => LocalizedString
					/**
					 * Obrázek je příliš veliký, maximální velikost obrázku je 15MB.
					 */
					too_large: () => LocalizedString
				}
				distance: {
					/**
					 * Zadejte prosím vzdálenost
					 */
					blank: () => LocalizedString
					/**
					 * Vzdálenost musí být kladné celé číslo
					 */
					invalid: () => LocalizedString
					/**
					 * Vzdálenost musí být kladné celé číslo
					 */
					negative: () => LocalizedString
				}
				elevation: {
					/**
					 * Převýšení musí být kladné celé číslo
					 */
					invalid: () => LocalizedString
					/**
					 * Převýšení musí být kladné celé číslo
					 */
					negative: () => LocalizedString
				}
				activity: {
					/**
					 * Prosím vyberte aktivitu.
					 */
					blank: () => LocalizedString
					/**
					 * Vybraná aktivita není platná.
					 */
					invalid: () => LocalizedString
				}
			}
		}
		state: {
			/**
			 * Zpracovává se
			 */
			pending: () => LocalizedString
			/**
			 * Schváleno
			 */
			accepted: () => LocalizedString
			/**
			 * Zamítnuto
			 */
			rejected: () => LocalizedString
		}
		errors: {
			/**
			 * Jiný administrátor změnil tuto aktivitu. Aktualizujte stránku a opakujte akci.
			 */
			mismatch_updated_at: () => LocalizedString
		}
	}
	extra_points: {
		/**
		 * Vzdálenost za 1 týden
		 */
		weekly_distance: () => LocalizedString
		/**
		 * Vzdálenost za 1 den
		 */
		daily_distance: () => LocalizedString
		/**
		 * Převýšení za aktivitu
		 */
		weekly_elevation: () => LocalizedString
	}
	account: {
		/**
		 * Účet
		 */
		title: () => LocalizedString
		/**
		 * E-mail
		 */
		email: () => LocalizedString
		/**
		 * Nové heslo
		 */
		password: () => LocalizedString
		/**
		 * Nové heslo znova
		 */
		password_repeat: () => LocalizedString
		/**
		 * Jméno
		 */
		first_name: () => LocalizedString
		/**
		 * Příjmení
		 */
		last_name: () => LocalizedString
		/**
		 * Fakulta
		 */
		faculty: () => LocalizedString
		/**
		 * Současné heslo
		 */
		old_password: () => LocalizedString
		/**
		 * Pokud některé údaje nesedí, kontaktujte nás na email vytrvale@ntis.zcu.cz
		 */
		invalid_info: () => LocalizedString
		/**
		 * Uložit
		 */
		save: () => LocalizedString
		/**
		 * Údaje byly úspěšně změněny
		 */
		success: () => LocalizedString
		/**
		 * Nebezpečná zóna
		 */
		danger_zone: () => LocalizedString
		emailing: {
			/**
			 * Přeji si dostávat emaily
			 */
			description: () => LocalizedString
			/**
			 * Odebírání emailů úspěšně změněno
			 */
			success: () => LocalizedString
			/**
			 * Nastala chyba při úpravě odběru emailů
			 */
			error: () => LocalizedString
		}
		'delete': {
			/**
			 * Smazat účet
			 */
			title: () => LocalizedString
			/**
			 * Smazání účtu je nevratné. Tato akce nelze vzít zpět.
			 */
			desc1: () => LocalizedString
			/**
			 * Po stisknutí tlačítka odstranit bude váš účet okamžitě zpracován ke smazání a všechna data budou trvale odstraněna.
			 */
			desc2: () => LocalizedString
			/**
			 * Jakmile účet smažete, již ho nelze obnovit.
			 */
			desc3: () => LocalizedString
			/**
			 * Měsíční Vytrvalec si bude nadále uchovávat Vaše sdílené aktivity za účelem uchování statistik uplynulých ročníků soutěže. Nebudou ovšem nijak spojeny s vaším stávajícím účtem.
			 */
			desc4: () => LocalizedString
			/**
			 * Odstranit
			 */
			submit: () => LocalizedString
			/**
			 * Účet odstraněn
			 */
			success: () => LocalizedString
			/**
			 * Nastala chyba při odstraňování účtu
			 */
			error: () => LocalizedString
		}
		/**
		 * Změna hesla
		 */
		password_change: () => LocalizedString
		errors: {
			email: {
				/**
				 * E-mail nesmí být prázdný
				 */
				blank: () => LocalizedString
				/**
				 * E-mail je neplatný
				 */
				invalid: () => LocalizedString
				/**
				 * Není co upravit
				 */
				no_update: () => LocalizedString
			}
			password: {
				/**
				 * Heslo nesmí být prázdné
				 */
				blank: () => LocalizedString
				/**
				 * Vaše heslo musí být minimálně "dostačující"
				 */
				weak: () => LocalizedString
			}
			old_password: {
				/**
				 * Současne heslo nesmí být prázdné
				 */
				blank: () => LocalizedString
				/**
				 * Současne heslo není správné
				 */
				mismatch: () => LocalizedString
			}
			password_repeat: {
				/**
				 * Nové hesla se musí shodovat.
				 */
				password_mismatch: () => LocalizedString
			}
		}
	}
	results: {
		/**
		 * Výsledky sezóny
		 */
		title: () => LocalizedString
		/**
		 * Výsledky
		 */
		results: () => LocalizedString
		week_picker: {
			/**
			 * Celá sezóna
			 */
			'0': () => LocalizedString
			/**
			 * První týden
			 */
			'1': () => LocalizedString
			/**
			 * Druhý týden
			 */
			'2': () => LocalizedString
			/**
			 * Třetí týden
			 */
			'3': () => LocalizedString
			/**
			 * Čtvrtý týden
			 */
			'4': () => LocalizedString
		}
		/**
		 * Zatím nejsou žádné výsledky pro tento týden či sezónu
		 */
		no_results: () => LocalizedString
		/**
		 * Fakulta
		 */
		faculty: () => LocalizedString
		/**
		 * Vzdálenost
		 */
		distance: () => LocalizedString
		/**
		 * Převýšení
		 */
		elevation: () => LocalizedString
		/**
		 * Body
		 */
		points: () => LocalizedString
		/**
		 * Celkem
		 */
		total: () => LocalizedString
		/**
		 * Účastníci podle fakult
		 */
		by_faculty: () => LocalizedString
		/**
		 * Počet
		 */
		count: () => LocalizedString
		/**
		 * TOP 3 účastníci
		 */
		top3: () => LocalizedString
		/**
		 * Zatím žádní
		 */
		no_top: () => LocalizedString
		/**
		 * Celkem vybráno
		 */
		total_gained: () => LocalizedString
		extras: {
			/**
			 * Extra body
			 */
			title: () => LocalizedString
			/**
			 * Jméno
			 */
			name: () => LocalizedString
			/**
			 * Fakulta
			 */
			faculty: () => LocalizedString
			/**
			 * Kategorie
			 */
			category: () => LocalizedString
			/**
			 * Aktivita
			 */
			activity: () => LocalizedString
			/**
			 * Výkon
			 */
			value: () => LocalizedString
			/**
			 * Body
			 */
			points: () => LocalizedString
		}
	}
	profile: {
		/**
		 * Aktivity
		 */
		submissions: () => LocalizedString
		loading: {
			/**
			 * Načítání statistik...
			 */
			statistics: () => LocalizedString
			/**
			 * Načítání aktivit...
			 */
			submissions: () => LocalizedString
		}
		/**
		 * Zatím nemáte žádné aktivity
		 */
		no_submissions: () => LocalizedString
	}
	reset: {
		/**
		 * Změna hesla
		 */
		title: () => LocalizedString
		/**
		 * Heslo
		 */
		password: () => LocalizedString
		/**
		 * Heslo znova
		 */
		password_repeat: () => LocalizedString
		/**
		 * Heslo bylo změněno
		 */
		success: () => LocalizedString
		/**
		 * Nastala chyba při změně hesla
		 */
		error: () => LocalizedString
		/**
		 * Změnit
		 */
		submit: () => LocalizedString
		errors: {
			/**
			 * Nové heslo nesmí být prázdné.
			 */
			blank: () => LocalizedString
			/**
			 * Hesla se musí shodovat.
			 */
			password_mismatch: () => LocalizedString
			/**
			 * Vaše heslo musí být minimálně "dostačující"
			 */
			weak: () => LocalizedString
		}
	}
	activity: {
		errors: {
			min_elevation: {
				/**
				 * Převýšení musí být kladné číslo
				 */
				negative: () => LocalizedString
			}
			'name,cz': {
				/**
				 * Název nesmí být prázdný
				 */
				min_length: () => LocalizedString
			}
			'name,en': {
				/**
				 * Název nesmí být prázdný
				 */
				min_length: () => LocalizedString
			}
			image: {
			}
		}
	}
	activities: {
		/**
		 * Běh a chůze
		 */
		'Běh/Chůze': () => LocalizedString
		/**
		 * Kolo, koloběžka a brusle
		 */
		'Kolo/Koloběžka': () => LocalizedString
		/**
		 * Celkové hodnocení
		 */
		total: () => LocalizedString
	}
	icons: {
		/**
		 * runner
		 */
		'Běh/Chůze': () => LocalizedString
		/**
		 * bike
		 */
		'Kolo/Koloběžka': () => LocalizedString
	}
	rules: {
		/**
		 * Pravidla
		 */
		title: () => LocalizedString
		/**
		 * Pravidla výzvy
		 */
		challenge_title: () => LocalizedString
		intro: {
			/**
			 * Vítejte
			 */
			step_title: () => LocalizedString
			/**
			 * Běhejte, choďte jezděte na kole, koloběžce, bruslích! Nejen, že uděláte něco pro svoje zdraví, ale přispějete na dobrou věc!
			 */
			content1: () => LocalizedString
			/**
			 * Kilometry, které soutěžící urazí se přemění na koruny a přispějeme tím na charitu.
			 */
			content2: () => LocalizedString
			/**
			 * Průběžný počet najetých a uběhnutých kilometrů fakult a mimofakultních pracovišť budeme vyhodnocovat po týdnu a každá fakulta či pracoviště mají možnost získat body do celkového hodnocení.
			 */
			content3: () => LocalizedString
		}
		disciplines: {
			/**
			 * Disciplíny
			 */
			step_title: () => LocalizedString
			/**
			 * Disciplíny
			 */
			title: () => LocalizedString
			/**
			 * Soutěž probíhá ve dvou disciplínách
			 */
			description: () => LocalizedString
		}
		progress: {
			/**
			 * Soutěž
			 */
			step_title: () => LocalizedString
			/**
			 * Průběh soutěže
			 */
			title: () => LocalizedString
			/**
			 * Doporučené aplikace
			 */
			recommended_apps: () => LocalizedString
			/**
			 * Uživatelé zaznamenávají své cílené pohybové aktivity prostřednictvím některé mobilní aplikace na svůj telefon a nahrají je na tento portál. Pro zaznamenávání cílových aktivit můžete využít např. aplikaci Garmin Connect, nebo Strava.
			 */
			content1: () => LocalizedString
			/**
			 * Každý týden se vyhodnocuje počet uražených kilometrů v daných disciplínách.
			 */
			content2: () => LocalizedString
			/**
			 * První tým dostane v každé disciplíně např. 15 bodů (podle počtu zůčastněných fakult a pracovišť), druhý 14 bodu, třetí 13 bodů, apod.
			 */
			content3: () => LocalizedString
			/**
			 * Bodování
			 */
			scoring: () => LocalizedString
			/**
			 * 1. místo: 15 bodů
			 */
			first_place: () => LocalizedString
			/**
			 * 2. místo: 14 bodů
			 */
			second_place: () => LocalizedString
			/**
			 * 3. místo: 13 bodů
			 */
			third_place: () => LocalizedString
			/**
			 * Celkovým vítězem výzvy se stane tým, který získá nejvíce bodů za 4 týdny v součtu obou disciplín.
			 */
			content4: () => LocalizedString
		}
		extra_points: {
			/**
			 * Extra body
			 */
			step_title: () => LocalizedString
			/**
			 * Extra body!
			 */
			title: () => LocalizedString
			/**
			 * Jednotlivci mohou pro svou fakultu získat extra body.
			 */
			intro: () => LocalizedString
			third_week: {
				/**
				 * Třetí týden
				 */
				title: () => LocalizedString
				/**
				 * pro jednotlivce, který uběhne/ujde/ujede nejvíce kilometrů v jednom dni v dané disciplíně.
				 */
				extra_one: () => LocalizedString
				/**
				 * pro jednotlivce s největším součtem kilometrů za celý týden v dané disciplíně.
				 */
				extra_two: () => LocalizedString
			}
			fourth_week: {
				/**
				 * Čtvrtý týden
				 */
				title: () => LocalizedString
				/**
				 * pro jednotlivce s největším převýšením v jedné aktivitě v dané disciplíně. Chůze více než 1000m a jízda více než 1500m.
				 */
				extra_one: () => LocalizedString
			}
			/**
			 * Extra bod
			 */
			extra_point: () => LocalizedString
			/**
			 * Extra body
			 */
			extra_points: () => LocalizedString
		}
		target_activities: {
			/**
			 * Aktivity
			 */
			step_title: () => LocalizedString
			/**
			 * Cílová pohybová aktivita
			 */
			title: () => LocalizedString
			/**
			 * Co JE cílová aktivita
			 */
			is_activity: () => LocalizedString
			/**
			 * Co NENÍ cílová aktivita
			 */
			not_activity: () => LocalizedString
			/**
			 * Jdu si zaběhat, zapnu si aktivitu a vypnu si aktivitu.
			 */
			activity1: () => LocalizedString
			/**
			 * Jdu se projít, zapnu si aktivitu a vypnu si aktivitu.
			 */
			activity2: () => LocalizedString
			/**
			 * Jedu na kole, zapnu si aktivitu a vypnu si aktivitu.
			 */
			activity3: () => LocalizedString
			/**
			 * Součet nachozených kilometrů za den (např. z krokoměru).
			 */
			activity4: () => LocalizedString
		}
	}
	footer: {
		/**
		 * Kontaktujte nás:
		 */
		contact: () => LocalizedString
		/**
		 * Správa webu
		 */
		managed_by: () => LocalizedString
	}
	faculties: {
		/**
		 * Fakulta aplikovaných věd
		 */
		FAV: () => LocalizedString
		/**
		 * Fakulta designu a umění Ladislava Sutnara
		 */
		FDU: () => LocalizedString
		/**
		 * Fakulta ekonomická
		 */
		FEK: () => LocalizedString
		/**
		 * Fakulta elektrotechnická
		 */
		FEL: () => LocalizedString
		/**
		 * Fakulta filozofická
		 */
		FF: () => LocalizedString
		/**
		 * Fakulta pedagogická
		 */
		FPE: () => LocalizedString
		/**
		 * Fakulta právnická
		 */
		FPR: () => LocalizedString
		/**
		 * Fakulta strojní
		 */
		FST: () => LocalizedString
		/**
		 * Fakulta zdravotnických studií
		 */
		FZS: () => LocalizedString
		/**
		 * Rektorát
		 */
		REK: () => LocalizedString
		/**
		 * Nové technologie - výzkumné centrum
		 */
		NTC: () => LocalizedString
		/**
		 * Univerzita třetího věku
		 */
		U3V: () => LocalizedString
		/**
		 * Ústav jazykové přípravy
		 */
		UJP: () => LocalizedString
		/**
		 * Knihovna
		 */
		KNIHOVNA: () => LocalizedString
		/**
		 * Centrum informatizace a výpočetní techniky
		 */
		CIV: () => LocalizedString
		/**
		 * ŠUZ Nečtiny
		 */
		SUZN: () => LocalizedString
		/**
		 * Ústav tělesné výchovy a sportu
		 */
		UTS: () => LocalizedString
	}
	server: {
		/**
		 * Server není dostupný, opakujte akci později.
		 */
		server_down: () => LocalizedString
	}
}

export type Formatters = {
	currency: (value: number) => unknown
	long_date: (value: Date) => unknown
}
