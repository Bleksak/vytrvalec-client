...
{:else}
    <article>
        <Heading>
            <h1>
                Úprava sezóny: {data.season.start.toLocaleString(getLocale(), {
                    dateStyle: 'medium',
                })}
            </h1>
        </Heading>
        ...
        <form method="post" action="/administration/season?/update" use:enhance={enhancer}>
            ...
            <fieldset>
                ...
                <div class="grid">
                    ...
                    <a
                        href="/administration/charity/{data.season.charity.id}?season={data.season
                            .id}"
                        role="button"
                        class="outline"
                    >
                ...
                {#each data.faculties as [id, faculty], i}
                    <div class="grid">
                        ...
                        <select name="faculty_mapping[{i}][parent]" id="faculty-{id}">
                            <option
                                selected={existingFacultyMapping.get(faculty.id) === null}
                                value={null}
                            >
                                Žádná
                            </option>
                            {#each data.faculties as [parent_id, parent_faculty]}
                                {#if parent_id !== id}
                                    <option
                                        value={parent_id}
                                        selected={existingFacultyMapping.get(faculty.id) ===
                                            parent_id}
                                    >
