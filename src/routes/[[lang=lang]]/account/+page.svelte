<script lang="ts">
    ...
    const currentUser: UserResponse = $derived(data.user!);
    ...
    const handleSubscribtionChange = async (
        ...
        const response = await emailSubscribeChange(event.currentTarget.checked, data.api);
        if (response.errors) {
            toastStore.add({
                type: 'error',
                ...
                <form method="post" action="/auth?/account" use:enhance={enhancer}>
                    ...
                    <fieldset>
                        ...
                        {#each errors.password ?? [] as error}
                            <span class="error">
                                {$LL.account.errors.password[
                                    error as keyof typeof $LL.account.errors.password
                                ]()}
                            </span>
                        {/each}
                        <input
